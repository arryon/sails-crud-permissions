...
function permits (req, compositeGrant) {
  ...
  if (flag === undefined) {
    req._sails.log.info('Permission service: undefined action ' + req.options.action + ', skipping.');
...
function authorized (req, permissions, sails) {
   ...
   */
  var compositeGrant = util.mergeGrants(permissions, sails);
  if (permits(req, compositeGrant)) {
    return compositeGrant;
  }
}

function findPermissions (req, userId) {
  return User.findOne({id: userId})
  .populateAll()
  .then(function (user) {
    //also populate the roles into req.session
    req.session.roles = _.pluck(user.associations.roles.value, 'name');
