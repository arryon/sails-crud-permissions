...
function findPermissions (userId) {
  ...
  .then(function (user) {
    ...
    return Promise.all([
      Promise.resolve(_.pluck(user[0].associations.roles.value, 'id')),
      Group.find({id: _.pluck(user[0].associations.groups.value, 'id')})
      .populateAll()
  ...
  .then(function (roles) {
    ...
    sails.log.debug('found permissions: ' + JSON.stringify(permissions));
    return permissions;
  });
}
